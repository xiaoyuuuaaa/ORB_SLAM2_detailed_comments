 ...
 * 
 * Step 1：纯定位模式下不插入关键帧，如果局部地图被闭环检测使用，则不插入关键帧
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    // mMaxFrames等于图像输入的帧率
    //  Step 3：如果距离上一次重定位比较近，并且关键帧数目超出最大限制，不插入关键帧 防止短时间内插入过多关键帧。
    ...
    //内点数mnMatchesInliers小于15，则不插入新的关键帧。内点数少表示跟踪效果差，不适合插入关键帧。

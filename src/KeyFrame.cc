...
//关键帧的构造函数
// 参数：Frame &F: 传入的Frame对象，这个对象包含了图像帧的所有信息。
// 	Map *pMap: 指向地图对象的指针，这个地图对象保存了所有的地图点和关键帧。
//	KeyFrameDatabase *pKFDB: 指向关键帧数据库的指针，用于存储和检索关键帧的特征描述符。
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB): 
    mnFrameId(F.mnId),  mTimeStamp(F.mTimeStamp), mnGridCols(FRAME_GRID_COLS), mnGridRows(FRAME_GRID_ROWS), // 网格的列数和行数，用于将图像划分成网格以加速特征匹配。
    ...
    mvInvLevelSigma2(F.mvInvLevelSigma2), mnMinX(F.mnMinX), mnMinY(F.mnMinY), mnMaxX(F.mnMaxX), // 图像的有效区域
